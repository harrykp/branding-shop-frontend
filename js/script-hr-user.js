...
function fetchAndRender(page = 1) {
  fetch(`/api/hr-user?page=${page}`, {
    ...
    .catch(err => console.error("Error loading hr-user:", err));
}

function renderTable(items) {
  const tbody = document.getElementById("hr-list");
  tbody.innerHTML = "";
...
function filterTable(query) {
  ...
  const rows = document.querySelectorAll("#hr-list tr");
  rows.forEach(row => {
