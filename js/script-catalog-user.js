...
function fetchAndRender(page = 1) {
  fetch(`/api/catalog-user?page=${page}`, {
    headers: {
    ...
    .catch(err => console.error("Error loading catalog-user:", err));
}
...
function filterTable(query) {
  ...
  const rows = document.querySelectorAll("#catalog-list tr");
  rows.forEach(row => {
