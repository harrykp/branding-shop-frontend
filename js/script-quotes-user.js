...
function fetchAndRender(page = 1) {
  fetch(`/api/quotes-user?page=${page}`, {
    headers: {
    ...
    .catch(err => console.error("Error loading quotes-user:", err));
}

function renderTable(items) {
  const tbody = document.getElementById("quote-list");
  tbody.innerHTML = "";
...
function filterTable(query) {
  ...
  const rows = document.querySelectorAll("#quote-list tr");
  rows.forEach(row => {
