...
function renderList(resource, records) {
  ...
  const filtered = records.filter(r =>
    columns.some(c =>
      String(r[c.key]||'').toLowerCase()
        .includes(st.filter.toLowerCase())
    )
  );
  ...
  // build search + new toolbar
  const toolbar = `
    <div class="d-flex justify-content-between align-items-center mb-3">
      <div class="input-group w-50">
        <input type="text" id="search-${resource}"
  ...
  for (let i=1;i<=totalPages;i++) {
    pages += `<li class="page-item ${i===st.page?'active':''}">
      <a class="page-link" href="#" onclick="goPage('${resource}',${i})">${i}</a>
    </li>`;
  }
  const paginator = `
    ...
    <div class="mb-4">
      <label>Per page:
        <select onchange="setPerPage('${resource}',this.value)">
          ${[5,10,20,50].map(n=>
            `<option ${n===st.perPage?'selected':''} value="${n}">${n}</option>`
          ).join('')}
        </select>
      </label>
    </div>`;
...
// --- CRUD actions & form rendering (unchanged from before) ---
