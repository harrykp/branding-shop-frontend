    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
      <div class="input-group" style="width:250px">
        ...
        <input type="text" class="form-control" placeholder="Search…"
               value="${s.search}" oninput="onSearch('${resource}',this.value)">
...
function renderForm(resource, record={}) {
  ...
  const fieldsHtml = columns.map(c => {
    ...
    return `<div class="mb-3"><label class="form-label">${c.label}</label>` +
           `<input id="f_${c.key}" class="form-control" type="${c.type||'text'}" value="${val}" ${c.readonly?'readonly':''} ${c.readonly?'':'required'} /></div>`;
  }).join('');

  app.innerHTML = `<h3>${isEdit?'Edit':'New'} ${resource.slice(0,-1)}</h3><form id="frm_${resource}">${fieldsHtml}` +
